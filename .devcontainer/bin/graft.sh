...
if [ "$FIRST_RUN" -eq 1 ] && [ "$DRY_RUN" != "true" ]; then
  ...
  if [ "$NON_INTERACTIVE" != "true" ] && [ -f "README.md" ]; then
    ...
    if prompt_confirm "Insert Codespaces badge & graft credit into README.md?" yes; then
      ...
      credit_line=' Codespace created with [evlist/codespaces-grafting <img src=".devcontainer/assets/icon.svg" width="64" height="64" style="float:right;max-width:96px;height:auto" alt="cs-grafting" /> ](https://github.com/evlist/codespaces-grafting)'
